<script lang="ts">
	...
	async function initialize() {
		generateWords(30, keys[$current_key_i]).then(() => {
			assignWraps();
			focusInput();
		});
		resetStores();
...
<div class="h-full flex flex-col items-center justify-center gap-8">
	{#if $appState == 'results'}
		<div class="flex flex-col items-center">
			...
			<div class="flex gap-4">
				<button class="btn" on:click={restart}>Újrakezdés</button>
				{#if $accuracy >= 80}
					<button class="btn" on:click={next}>Következő</button>
				{/if}
			</div>
		</div>
	{/if}
	...
	{#if $appState != 'results'}
		<button class="btn" on:click={() => modal.showModal()}>Lecke kiválasztása</button>
